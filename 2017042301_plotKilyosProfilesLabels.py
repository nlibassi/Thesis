...
for pname in profNames:
	...
	plt.text(-150, Shore2001 + 0.5, str(Shore2001) + 'm', color='b', path_effects=[pe.withStroke(linewidth=3,
                                                                                  foreground="w")])
	plt.text(-150, Shore2017 - 0.5, str(Shore2017) + 'm', color='g', path_effects=[pe.withStroke(linewidth=3,
                                                                                  foreground="w")])
	...
	with open(outFile, 'a') as out:
		...
		for i in indexListStg:
			if distList[i[0]:i[1]]:
				...
				labelx = (xmin + xmax)/2
				labely = (ymin + ymax)/2
				if areaList[indexListStg.index(i)] > 5 or areaList[indexListStg.index(i)] < -5:
					areaLabel = '$ %.2f m^2$' % areaList[indexListStg.index(i)]
					labelList.append([labelx, labely, areaLabel])
